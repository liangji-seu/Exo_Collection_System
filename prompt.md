...
- 三个 IMU 各一个固定长度循环曲线窗口；左右两个电机编码器各一个固定长度循环曲线窗口。新帧从左到右原位覆盖，到达右边界后从左侧继续，红色竖线标记最新更新帧。
- 所有实时曲线的横轴和纵轴在首次有效数据确定同模态共同范围后均须锁定，不能随采集增长、整体滚动、自动缩放或由用户缩放改变。
- IMU 多设备和编码器左右通道必须通过带标签的多通道共享内存 preview payload 传输，不能重复单路数据或把批量数组直接塞进 UI 进程队列。
- 全零等格式级异常可直接报警；依赖真实标定的弱信号、边界或滑移阈值不得擅自虚构。
- Collector 不显示同步/状态/告警时间线图；同步状态、trigger 计数、首触发和质量保留在左侧同步面板，事件继续进入内部有界记录和告警区。
