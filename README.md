...
Data Studio 使用 Manifest 与 SQLite Catalog 建立 `项目 → 受试者 → 工况 → Session → 模态数据集` 数据树。上传时可选择项目、受试者、工况、Session 或单个 Trial，系统会上传该节点下全部 `FINALIZED` Trial，并原样保留它们相对于本地 `data/` 的目录结构。首页“同步云端状态”会只读核对远端同路径文件的 SHA-256，并用绿色、红色、橙色和紫色分别标记已上传、未上传、部分缺失和内容冲突；不会因状态同步而上传、下载、覆盖或删除文件。
