...
Collector 写盘 gate 打开期间支持两类人工事件：`<` 为“受试者标签”，`>` 为“工作人员标签”。按键到达 UI 时立即同时记录 `host_monotonic_ns` 和审计用 `host_utc_ns`，经独立有界控制队列送入 Collector Worker，不与任何高吞吐模态的原始数据队列混用。写盘前或停止边界后的按键不进入 Trial。

事件以追加式 `raw/prompt_labels.jsonl` 保存，每行至少包含 Schema 版本、Trial UUID、连续 sequence、来源、中文标签名、按键、主机单调时间和 UTC。文件每次追加后刷盘；最终化时作为不可变 RAW Artifact 纳入 Manifest 和 `checksums.sha256`。若本 Trial 没有标签，则不发布空 Artifact。标签不是 Modality Adapter，不参与设备 READY 判定或模态丢失质检。

Collector 左侧表格分别显示两类标签计数。IMU 和编码器循环预览在按键发生的环形位置绘制细红竖线；当下一圈数据真正覆盖该位置时才删除。Collector 的超声是当前单帧 A-scan，因此不绘制时间标签。Data Studio 按公共 Trial 时间轴在超声瀑布图、IMU 和编码器回放上显示相同标签；虚线表示受试者、实线表示工作人员，悬停可查看标签和时间。
        ...
        trials/
          trial_uuid/
            ...
            raw/
              ...
              encoder.h5
              sync_pulse.h5
              prompt_labels.jsonl      # 仅在本 Trial 存在人工标签时发布
              external/
            derived/
              preview/
              alignment.json
...
- 超声原始数据写盘优先级高于预览；
- 预览落后时只丢预览帧，不丢原始帧；
- 右侧预览采用模态键注册的可停靠工作区；每个面板可拖动、缩放、浮动、关闭和从“添加窗口”重新显示，面板显隐不改变 Adapter 或记录支路生命周期；
- 停靠位置、浮动状态和显隐状态通过 `QSettings` 持久化；关闭面板后后台预览仍可继续更新其有界显示缓存；
- 动捕面板直接显示全部 Marker 的最新 XYZ 表格；测力台面板接受 `fx/fy/fz/mx/my/mz` 通用预览事件，但在真实测力台 Adapter 接入前不属于 Collector 必需采集模态；
...
- 写盘期间捕获 `<`（受试者）和 `>`（工作人员）人工标签；IMU 与编码器预览在对应环形位置显示细红竖线，直到下一圈数据覆盖该位置；
...
所有固定窗口回放采用真正的环形覆盖：到达窗口末端后保留上一圈尚未覆盖的内容，只用新一圈已经到达的样本逐列替换，不在换圈瞬间清空整张图。人工标签红线遵循相同生命周期。
...
- 禁止在日志中打印密码；
- 禁止保存明文密码；需要记住密码时仅使用 Windows 凭据管理器；
- 首次连接显示服务器主机指纹并要求确认；
...
- 两个模态分别写入 `raw/mocap.h5` 和 `raw/emg.h5`，保留帧号/设备时间、主机单调
  时间、UTC 审计时间和源序列号。
- 实时预览属于有损抽取：动捕消息保留全部 Marker 的最新 XYZ 供表格显示，循环
  曲线通道仍限制为前 8 个以约束 IPC 体积；EMG 显示前 16 个通道。预览限流或
  界面卡顿不得影响原始记录支路。
