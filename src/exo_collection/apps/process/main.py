...
from exo_collection.logging_setup import process_log_path, setup_process_logging

_log = logging.getLogger(__name__)


def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="exo-process")
    parser.add_argument(
        "--smoke-test",
        action="store_true",
...
def _run_ui(
    ...
    *,
    ...
    QApplication.setApplicationName("Exo Process")
    app = QApplication.instance()
    if app is None:
        app = QApplication(["exo-process", *arguments])

    window = ProcessWindow(data_root, settings=settings)
...
def main(
    ...
    *,
    ...
    setup_process_logging(level=logging.DEBUG, console=True)
    logger = logging.getLogger("exo_collection.process.main")
    logger.info("Exo Process application starting; log_file=%s", process_log_path())
    ...
    if options.smoke_test:
        ...
        with TemporaryDirectory(prefix="exo-process-smoke-") as directory:
